...
#View for transferring a patient to a different hospital
@method_decorator(login_required, name='dispatch')
...
def dashboard(request):
    ...
    if user.role == models.Registration.DOCTOR:
        ...
        return render(request, template_name, context={'user':user, 'appts':appt, 'msgs':messages, 'records':records})
    elif user.role == models.Registration.NURSE:
        #If they are checked in to a hospital, remove any that aren't in the doctor's hospital
        appt = models.Appointment.objects.all()
        patients = models.Registration.objects.filter(role='PT').order_by('in_hospital')
        template_name = "system/nurse_dashboard.html"
        return render(request, template_name, context={'user':user, 'ptnts':patients, 'msgs':messages, 'appts':appt})
        ...
        return render(request, template_name, context={'user':user, 'sys':system_act, 'users':users, 'hosp':hospitals, 'msgs':messages, 'stat':stats})
...
def create_appt(request):
    ...
    if user.role == models.Registration.DOCTOR:
        ...
        return ApptCreateNurse.as_view()(request)
    return HttpResponseForbidden()
...
def update_appt(request, pk):
    ...
    if user.role == models.Registration.DOCTOR:
        ...
        return ApptUpdateNurse.as_view()(request, pk=pk)
    return HttpResponseForbidden()
...
#View to create an appointment as a nurse
@method_decorator(login_required, name='dispatch')
class ApptCreateNurse(edit.CreateView):
    model = models.Appointment
    fields = ['doctor', 'patient', 'date_scheduled']
    template_name = "system/appointment_form.html"
    success_url = reverse_lazy("system:dashboard")

#View to update an appointments as a nurse
@method_decorator(login_required, name='dispatch')
class ApptUpdateNurse(edit.UpdateView):
    model = models.Appointment
...
def view_records(request, pk=None):
    ...
    if user.role == models.Registration.DOCTOR or user.role == models.Registration.NURSE:
        ...
        return render(request, 'system/record_list.html', context={'records':records, 'is_doctor': isDoctor, 'is_patient': False})
    elif user.role == models.Registration.PATIENT:
        records = models.Record.objects.filter(patient=user, patient_readable=True)
        return render(request, 'system/record_list.html', context={'records':records, 'is_patient': True, 'is_doctor': False})
    return HttpResponseForbidden()
...
def GenerateStatisticsContext(request):
    stats = Context()
    patients = models.Registration.objects.filter(role=models.Registration.PATIENT)
    doctors = models.Registration.objects.filter(role=models.Registration.DOCTOR)
    nurses = models.Registration.objects.filter(role=models.Registration.NURSE)
    hospitals = models.Hospital.objects.all()
    stats['tot_pat'] = patients.count()
    stats['cur_pat'] = patients.filter(in_hospital=True).count()
    stats['tot_doc'] = doctors.count()
    stats['tot_nrs'] = nurses.count()
    stats['tot_hosp'] = hospitals.count()
    if nurses.count() != 0 or doctors.count() != 0:
        ratio = patients.count() / (nurses.count() + doctors.count())
        ratio = float("{0:.2f}".format(ratio))
    else:
        ratio = "No Doctors or Nurses"
    stats['stf_rat'] = ratio
    return stats
